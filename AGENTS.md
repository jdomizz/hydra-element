...
- `pnpm build` — bundles two artifacts: `dist/hydra-element.js` (main entry) and `dist/context.js` (`hydra-element/context` — `createContext`/`hydraEval`/`userCodeLine`). TypeScript declarations are emitted by `tsc` (`dist/index.d.ts`, `dist/context.d.ts`); `postbuild` asserts the two public `.d.ts` entry points exist. The internal `src/core/` layer is bundled into the main entry, not exported as a subpath.
...
- WTR + the Vite plugin occasionally fail a file with "Failed to fetch dynamically imported module" on the first run after files are added/removed (stale dep cache) — a clean re-run passes. If a fresh `playground/editor-panel-extensions.spec.js` test fails on first run, re-run once before debugging.
