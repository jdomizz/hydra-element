...
function compatNoteFor(entry, errors, black) {
  if (entry.compatNote) return entry.compatNote
  if (errors.length > 0) {
    const first = errors[0].slice(0, 80)
    const suffix = errors[0].length > 80 ? '…' : ''
    return `console errors: ${first}${suffix}`
  }
  if (black) {
    return 'canvas rendered all-black — entry likely needs additional setup (audio input, camera, backend, etc.)'
  }
  return ''
}
...
async function writeMatrix(results) {
  ...
  lines.push(
    `> Generated by \`scripts/check-extensions.mjs\` against the catalog data in \`playground/extensions.js\`.`
  )
  ...
  for (const { entry, ok, errors, black, shotPath, logPath } of results) {
    ...
    const compatNote = compatNoteFor(entry, errors, black)
  ...
  lines.push(
    '- `works-with-notes` — the demo loaded but has caveats (audio input, backend dependency, UI overlay outside the cell). The note column explains.'
  )
  lines.push(
    '- `not-yet` — the demo cannot run with the current `src/globals.js` published set. See §5 of `.opencode/specs/hydra-element/active/playground-extensions-catalog.md` for the bridge survey findings.'
  )
  lines.push('')
  lines.push('## Caveats')
  lines.push('')
  lines.push(
    '- This pass fetches 29 CDN scripts (metagrowing, jsDelivr, unpkg, statically.io, fentonia.com, emptyfla.sh). Some are large (shader-park-core is ~2.8 MB, total-serialism ~2 MB). A network failure mid-run leaves the affected entry labeled by the static analysis from `playground/extensions.js` — re-run when the network is healthy.'
  )
  lines.push(
