...
- `<hydra-element>` auto-injects a FOUC guard (`hydra-element:not(:defined) { display:none }`) at module load time, so raw `textContent` code never flashes on screen before the canvas renders. The injected `<style>` is idempotent and has `data-hydra-fouc` for easy inspection. For guaranteed zero-flash on the very first paint (before any JS runs), users can add a one-liner `<style>hydra-element:not(:defined){display:none}</style>` in their `<head>` — documented in the README.
