...
- **Rearchitect into three layers (TypeScript).** The element splits into a
  ...
  adapter that wires them. New `hydra-element/core` subpath (`createHydraCore`,
  `hydraEval`, `userCodeLine`, a scheduler-injectable `Loop`) — consumable from
  Node against an injected engine factory. Sources are now TypeScript (strict)
  with tsc-emitted declarations (`dist/index.d.ts`, `dist/core/index.d.ts`,
  `dist/eval.d.ts`); the public contract is unchanged (tag, attributes,
  properties, events, `./eval` subpath). Behavioral deltas: seed code (element
