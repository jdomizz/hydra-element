...
- **`hydra-eval` failure detail gains a best-effort `line` field.** When an eval fails, the `hydra-eval` detail now carries `line` (1-based user-code line) alongside `{ success: false, error }`. The user's code runs inside a `new Function` wrapper (`src/eval.js`), whose two synthetic prologue lines offset Chromium stack positions by +2; the new `userCodeLine()` export maps the first user-code frame back to the user's own numbering and validates it against the code's line range. `undefined` when the error carries no parseable position (syntax errors, non-Error rejections); success details carry no `line` field. Declared in `src/hydra-element.d.ts` (`HydraEvalDetail.line?`) and `src/eval.d.ts`. Additive — existing consumers reading `{ success, error }` are unaffected. Spec: `hydra-editor/active/editor-rich-features.md` §5.

- **Playground wires `hydra-eval` failures to the editor's inline error bar.** `<editor-panel>` (`playground/components/editor-panel.js`) now listens for `hydra-eval` on `document` and routes events from the active target to the editor: failures call `editor.markError({ line, message })` (line from the event detail, message from `error.message` or `String(error)`), successes call `editor.clearErrors()`, and events from non-active slots are ignored — the same active-target routing the panel already uses for `target-change`. Depends on `markError`/`clearErrors` landing in the `hydra-editor` package (parallel branch; resolved here via the `file:` devDependency). New spec `playground/editor-error-wiring.spec.js` (spy-based, primitive `callCount` assertions per the WTR sinon-chai hang quirk) — it fails at an API precondition until the parallel dist is installed; expected pending state. Spec: `hydra-editor/active/editor-rich-features.md` §5.
...
- **Rearchitect into three layers (TypeScript).** The element splits into a
  ...
  `webglcontextlost` is recoverable, and `webglcontextrestored` is awaited
  before re-initialization); global-mode code now evaluates through the same
  scoped `hydraEval` proxy as non-global mode (the old `hydra-synth` sandbox
  branch was dropped — the core has no sandbox path).

- **ο extraction (2026-09-01):** The `<hydra-editor>` element + Hydra config extract from `hydra-element` into a standalone repo `hydra-editor` (unpublished, AGPL-3.0-or-later). `hydra-element` 0.7.0 ships **without** the editor subpath. Removed: `src/editor/` (editor.js, hydra-config.js, index.js, specs, d.ts), `src/hydra-editor.d.ts`, `./editor` export from package.json, editor lib entry from vite.config.js, editor devDeps (@jdomizz/truss-editor, codejar, prismjs). Added: `hydra-editor: file:../hydra-editor` as devDependency. Playground imports `<hydra-editor>` from the local repo. Spec: `hydra-editor/backlog/hydra-editor-extraction.md`.
...
- **Playground polish pass** (per `active/playground-usability-audit.md` findings, wave-2 menu): focus rings restored on the 4 cells + on shadow-DOM buttons (`.cell.is-active:focus-visible`, `<multi-log>` button + pre `:focus-visible` rings); `<multi-log>`'s shadow `.btn` styles added (the clear button was previously browser-default-styled — a light button on a dark theme); a header strip in the playground (`<header class="app__header">` with `<code>&lt;hydra-element&gt;</code>` title + tagline + github/readme links) lands the onboarding affordance the audit flagged as the single biggest "impress" lift. The `.app` grid now sizes to `calc(100vh - 40px)` so the header doesn't shrink the 3-column playground. No new features — strictly fixes + polish.
