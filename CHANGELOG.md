...
- Cross-link from the playground to the gallery (`<a href="./gallery.html">multi-instance gallery →</a>` at the top of `.app__left`), closing the asymmetry with `gallery.html`'s existing "← back to playground" link. Styled consistently with `.gallery__links` in `playground/css/gallery.css`.
