...
| 22  | hydra-vertex               | extension | works            | bridge publishes `window.hydraSynth` as an alias for `_hydra` (same hydra-synth instance; bridge-globals-unification.md).                                                                                                                  |
| 23  | hydra-datamosh             | extension | works            | bridge publishes `window.hydraSynth`; datamosh's `params.hydra                                                                                                                                                                             |     | window.hydraSynth` fallback path now resolves. |
...
- The two `not-yet` entries (hydra-vertex #22, hydra-datamosh #23) are now `works` after the bridge-globals-unification.md fix lands. The `window.hydraSynth` alias is published by `src/globals.js` for the duration of the `loadScript` bridge (and permanently in global mode).
